inputstr = 1
local orig_search_key_press = AchievementListGui.search_key_press
...
Hooks:PreHook(AchievementListGui, "update_key_down", "alg_update_key_down_cn_backspace", function(self, o, k, ...)
    local match = false
    for i = 97, 122 do 
...
Hooks:PreHook(AchievementListGui, "enter_text", "alg_enter_text_cn_backspace", function(self, o, s, ...)
...
Hooks:PostHook(AchievementListGui, "enter_text", "alg_enter_text_cn_insert", function(self, o, s, ...)
    if s and self._inputing_past and tostring(#s) % 3 == 0 then
	    local ns, ne = self._search.text:selection()
		if self._inputing_past and #self._search.text:text() < 28 then
...
Hooks:PreHook(AchievementListGui, "search_key_release", "alg_key_release_cn_backspace", function(self, o, k, ...)
    inputstr = 0
end) 
